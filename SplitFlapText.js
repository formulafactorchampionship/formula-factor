...
const SplitFlapText = ({
  ...
  const tileElements = tiles.map((tile, index) => {
    const children = [
      React.createElement(
        'span',
        { className: 'split-flap-text__half split-flap-text__half--top', key: 'top' },
        React.createElement(
          'span',
      ...
      React.createElement(
        'span',
        { className: 'split-flap-text__half split-flap-text__half--bottom', key: 'bottom' },
        React.createElement(
          'span',
          { className: 'split-flap-text__char' },
    ...
    if (tile.flipping) {
      children.push(
        React.createElement(
          'span',
          { className: 'split-flap-text__flap split-flap-text__flap--front', key: `front-${index}-${tile.tick}` },
          React.createElement(
            'span',
            { className: 'split-flap-text__char' },
            tile.current === ' ' ? '\u00A0' : tile.current
        ...
        React.createElement(
          'span',
          { className: 'split-flap-text__flap split-flap-text__flap--back', key: `back-${index}-${tile.tick}` },
          React.createElement(
            'span',
            { className: 'split-flap-text__char' },
            tile.next === ' ' ? '\u00A0' : tile.next
    ...
    return React.createElement(
      'span',
      { className: 'split-flap-text__tile', 'aria-hidden': 'true', key: `${index}-${tiles.length}` },
      ...children
    );
  });
...
function SingleFlapUnit({ val, tileColor, textColor, fontSize = 64, tileRadius = 8, gap = 6 }) {
  return React.createElement(SplitFlapText, {
    text: String(val || '00').padStart(2, '0'),
    padTo: 2,
    charset: 'numeric',
    flipsPerChar: 0,
    flipDuration: 0.15,
...
export function initCountdownSplitFlaps() {
  const units = [
    { id: 'days-flap', type: 'days', color: '#0d131f', text: '#ffffff' },
    { id: 'hours-flap', type: 'hours', color: '#0d131f', text: '#ffffff' },
    { id: 'mins-flap', type: 'mins', color: '#0d131f', text: '#ffffff' },
    { id: 'secs-flap', type: 'secs', color: '#0d131f', text: '#f5e29f' }
  ];
  ...
  units.forEach(u => {
    ...
    if (el) {
      ...
      roots[u.type] = { root, color: u.color, text: u.text, lastVal: null };
      root.render(
        React.createElement(SingleFlapUnit, {
          val: '00',
          tileColor: u.color,
          textColor: u.text
        })
  ...
  window.updateCountdownFlaps = (days, hours, mins, secs) => {
    const values = { days, hours, mins, secs };
    Object.keys(values).forEach(k => {
      if (roots[k]) {
        const valStr = String(values[k]).padStart(2, '0');
        if (roots[k].lastVal !== valStr) {
          roots[k].lastVal = valStr;
          roots[k].root.render(
            React.createElement(SingleFlapUnit, {
              val: valStr,
              tileColor: roots[k].color,
              textColor: roots[k].text
            })
          );
        }
      }
